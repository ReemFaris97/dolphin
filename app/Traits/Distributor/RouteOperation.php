...
trait RouteOperation
{
    ...
    public function RegisterBill($request, RouteTrips $trip)
    {
        ...
        foreach ($request->products as $item) {
            ...
            $trip_report->products()->create([
                ...
                "price" =>
                    optional(
                        ClientClassProduct::query()
                            ->where("product_id", $item["product_id"])
                            ->where("client_class_id", $class_id)
                            ->first()
                    )->price ?? $product->price,
                "product_id" => $product->id,
    ...
    public function RegisterRouteReport($request)
    {
        ...
        $user_routes = DistributorRoute::where(
            "user_id",
    ...
    {
        $damage_store =
            Store::ofDistributor(auth()->id())
                ->where("for_damaged", 1)
                ->first() ?? User::find(auth()->id())->createDamageStore();
        if ($request->hasFile("image")) {
