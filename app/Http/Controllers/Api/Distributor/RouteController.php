...
class RouteController extends Controller
{
    ...
    {
        $routes = DistributorRoute::with([
            "trips" => fn($trips) => $trips->with("client"),
        ])
        ...
        $active_route = DistributorRoute::with([
            "trips" => fn($trips) => $trips->with("client"),
        ])
            ->where("user_id", auth()->id())
            ->where(["is_available" => 1])
        ...
        return $this->apiResponse([
            "active_route" =>
                $active_route != null
                    ? new MapRoutesResource($active_route)
                    : null,
    ...
    public function show(Request $request, $id)
    {
        $trips = RouteTrips::query()
            ...
            ->where("route_id", $id)
            ->when($request->lat != null && $request->lng != null, function (
                $q
            ) use ($request) {
    ...
    public function attachProducts(Request $request)
    {
        $request["store_id"] =
            $request->store_id ??
            optional(optional(auth()->user())->car_store)->id;
        $request["products"] = json_decode($request->products, true);
        ...
        return $this->apiResponse([
            ...
            "bill" => url(
                "/api/distributor/bills/print_bill/" . encrypt($trip_report->id)
            ),
        ]);
    }
    ...
    public function print_bill($id)
    {
        ...
        return $pdf->download(
            Str::snake("{$bill->product_total()} $bill->created_at") . ".pdf"
        );
    ...
    public function lastBill(Request $request)
    {
        ...
        return $this->apiResponse([
            ...
            "bill" => url(
                "/api/distributor/bills/print_bill/" . encrypt($bill->id)
            ),
        ]);
    }
    ...
    public function AddClientToRoute(Request $request, $route_id)
    {
        ...
        $request["route_id"] = $route_id;
        $request["round"] =
            optional(DistributorRoute::find($route_id))->round ?? 0;
        ...
        return $this->apiResponse([
            "msg" => "تم اضافه العميل الى المسار بنجاح",
