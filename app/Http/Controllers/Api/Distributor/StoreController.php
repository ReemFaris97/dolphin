...
class StoreController extends Controller
{
    ...
    {
        $stores = StoreTransferRequest::where([
    ...
    public function AcceptTransferRequest(Request $request, $id)
    {
        ...
        if ($request["signature"] != $store->signature) {
            return $this->SignaturenotFoundResponse();
        }
        if (!$store) {
    ...
    {
        $cars = DistributorCar::where("is_active", 1)
    ...
    {
        $product_quantities = Store::findOrFail($id)
            ->totalQuantities()
            ->paginate();

        $products = collect($product_quantities->items())->map(function (
            $products_quantities
        ) {
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            "products" => "required",
            "signature" =>
                "nullable|string|required_if:distributor_store_id,!=," .
                auth()->id(),
        ...
        $distributor_store_exist = Store::Where(
            "distributor_id",
            $request->distributor_id
        )->first();
        if (!$distributor_store_exist) {
            return $this->apiResponse(
                null,
                "المندوب المحدد ليس لديه مستودع",
                400
            );
        }
        $request["is_confirmed"] = 0;
        if ($request->distributor_id == auth()->user()->id) {
            // return $this->apiResponse(null, 'لا يمكنك ارسال واستقبال الاصناف الى و من نفسك', 400);
