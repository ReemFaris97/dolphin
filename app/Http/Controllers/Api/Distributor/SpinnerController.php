...
class SpinnerController extends Controller
{
    ...
    {
        $distributors = User::available()
            ->where("is_distributor", 1)
            ->get();
        return $this->apiResponse(
            DistributorSpinnerModelResource::collection($distributors)
        );
    }
    ...
    {
        $distributors = DistributorTransaction::receiverUser(auth()->user()->id)
            ->where("received_at", null)
            ->get();

        return $this->apiResponse(
            TransactionsSpinnerModelResource::collection($distributors)
        );
    }
    ...
    {
        if (is_null($store_id)) {
            $distributors = Product::withClientPrice(
                request()->client_id
            )->get();
        } else {
            $distributors = Product::whereHas("stores", function (
                Builder $builder
            ) use ($store_id) {
                $builder->where("store_id", $store_id);
            })
                ->withClientPrice(request()->client_id)
                ->get();
            ...
            $distributors = Product::with([
                ...
                ->whereHas("stores", function (Builder $builder) use (
                    $store_id
                ) {
                    $builder->where("store_id", $store_id);
                })
                ...
                ->map(function ($product) {
                    $product->store_quantity =
                        optional(
                            optional($product->quantities)->where(
                                "product_id",
                                $product->id
                            )
        ...
        return $this->apiResponse(
            ProductsSpinnerModelResource::collection($distributors)
        );
    }
    ...
    {
        $expenditure_clause = ExpenditureClause::where("is_active", 1)
            ->where("expenditure_type_id", $id)
            ->get();
        return $this->apiResponse(
            GeneralModelResource::collection($expenditure_clause)
        );
    }
    ...
    public function getExpenditureTypes()
    {
        ...
        return $this->apiResponse(
            GeneralModelResource::collection($expenditure_types)
        );
    }
    ...
    public function getProductByBarCode(Request $request)
    {
        ...
        $product = Product::with([
            "quantities" => function ($q) use ($request) {
        ...
        $product->store_quantity = $product->quantities
            ->where("product_id", $product->id)
            ->sum("total_quantity");

        if (!$product || $product->store_quantity == 0) {
            return $this->notFoundResponse();
        }
    ...
    {
        return $this->apiResponse(
            GeneralModelResource::collection(ClientClass::Active()->get())
        );
    }
}
