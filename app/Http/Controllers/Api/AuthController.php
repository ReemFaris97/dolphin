...
class AuthController extends Controller
{
    ...
    public function login(Request $request, $role = null)
    {
        ...
        if (!($token = JWTAuth::attempt($credentials))) {
            return $this->apiResponse(
                null,
                "البيانات التي ادخلتها غير صحيحة",
                406
            );
        }
        ...
        if (!is_null($user->blocked_at)) {
            return $this->apiResponse(
                null,
                "لقد تم حظر عضويتك الرجاء التواصل مع الاداره",
                420
            );
        }

        if ($role == "supplier" && $user->is_supplier == 0) {
            return $this->apiResponse(null, "عفواً المستخدم ليس مندوب", 401);
        }
    ...
    public function UpdateProfile(Request $request)
    {
        ...
        $rules = [
            ...
            "bank_account_number" =>
                "sometimes|numeric|unique:users,bank_account_number," .
                $user->id,
        ];
        $validation = $this->apiValidation($request, $rules);
        if ($validation instanceof Response) {
