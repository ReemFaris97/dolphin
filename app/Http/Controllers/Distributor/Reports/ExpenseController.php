...
class ExpenseController extends Controller
{
    public function __construct()
    {
        ...
        view()->share(
            "clauses",
            ExpenditureClause::query()->pluck("name", "id")
        );
    }
    public function index(Request $request)
    {
        ...
        if (
            $request->has("expenditure_type_id") &&
            $request->expenditure_type_id != null
        ) {
            $query = $query->where(
                "expenditure_type_id",
                $request->expenditure_type_id
            );
        }

        if (
            $request->has("expenditure_clause_id") &&
            $request->expenditure_clause_id != null
        ) {
            $query = $query->where(
                "expenditure_clause_id",
                $request->expenditure_clause_id
            );
        }
