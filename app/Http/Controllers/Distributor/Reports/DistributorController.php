...
class DistributorController extends Controller
{
    ...
    {
        view()->share('distributors', User::query()->distributor()->pluck('name', 'id'));
    }
    ...
    public function show(Request $request)
    {
        ...
        $products_with_total_quantity = $products_group->map(function ($product_sells) {
        ...
        $trips_per_month = $trips_during_the_period->groupBy(function ($trips_during_period) {
            return $trips_during_period->created_at->format('Y-m');
        });

        //cash and affiliate;
        $distributor = User::query()->distributor()->findOrFail($request->distributor_id);

        $total_per_month = $trips_per_month->map(function ($month_trips, $month) use ($distributor) {
