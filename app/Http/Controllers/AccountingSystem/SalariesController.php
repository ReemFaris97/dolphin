    ...
    public function index(Request $request)
    {
        $users = User::with([
            ...
            ->transform(function ($q) {
                ...
                $newData["bonus"] = $q->bonus_discount
                    ->where("type", "bonus")
                    ->sum("value");
                $newData["discount"] = $q->bonus_discount
                    ->where("type", "discount")
                    ->sum("value");
                $newData["allowance"] = $q->allowances->sum("pivot.value");
                $newData["debts"] = $q->debts
                    ->load("payments")
                    ->transform(function ($debt) {
                        $debt["_payments"] = collect($debt->paymentWithPayed())
                            ->where("is_current", true)
                            ->where("payed", false);
                        $debt["total_amount"] = $debt["_payments"]->sum(
                            "amount"
                        );
                        return $debt;
                    });
                $newData["total"] =
                    ...
                    $newData["allowance"] -
                    $newData["discount"] -
                    $newData["debts"]->sum("total_amount");
                $newData["total_without_debts"] =
                    $newData["salary"] +
                    $newData["bonus"] +
                    $newData["allowance"] -
                    $newData["discount"];
        ...
        return view(
            "AccountingSystem.salaries.index",
            compact("users", "date")
        );
    }
