...
class StoreTransactionController extends Controller
{
    ...
    public function productsingle(Request $request)
    {
        ...
        return response()->json([
            "status" => true,
            "data" => view(
                "AccountingSystem.stores.product",
                compact("products", "store")
            )->render(),
        ]);
    }
    ...
    {
        $store_product = AccountingProductStore::where("store_id", $id)
            ->pluck("product_id", "id")
            ->toArray();
        ...
        return response()->json([
            "status" => true,
            "data" => view(
                "AccountingSystem.stores.store_products",
                compact("products")
            )->render(),
        ]);
    }
    ...
    public function productsettlement(Request $request)
    {
        ...
        return response()->json([
            "status" => true,
            "data" => view(
                "AccountingSystem.stores.product_settlement",
                compact("products", "store")
            )->render(),
        ]);
    }
    ...
    public function productdamage(Request $request)
    {
        ...
        return response()->json([
            "status" => true,
            "data" => view(
                "AccountingSystem.stores.product_damaged",
                compact("products", "store")
            )->render(),
        ]);
    }
    ...
    public function transaction(Request $request, $id)
    {
        ...
        $product_store_form = AccountingProductStore::where("store_id", $id)
            ->where("product_id", $request["product_id"])
            ->first();
        if ($product_store_form->quantity - $request["quantity"] >= 0) {
            $product_store_form->update([
                "quantity" =>
                    $product_store_form->quantity - $request["quantity"],
            ]);
        }
        /////////update product_store_to_quantity

        $product_store_to = AccountingProductStore::where(
            "store_id",
            $request["store_to"]
        )
            ->where("product_id", $request["product_id"])
        ...
        if (isset($product_store_to)) {
            $product_store_to->update([
                "quantity" =>
                    $product_store_to->quantity + $request["quantity"],
            ]);
        } else {
        ...
        alert()
            ->success("تم النقل من المستودع بنجاح !")
            ->autoclose(5000);
        return back();
    }
    public function transactions(Request $request)
    {
        ...
        $massage = [
            "quantity.required" =>
                "اختر المستودع المحول منه ثم الاصناف المحولة",
        ];
        ...
        foreach ($merges as $merge) {
            ...
            /////////update product_store_form_quantity
            $product_store_form = AccountingProductStore::where(
                "store_id",
                $store_form
            )
            ...
            if ($product_store_form->quantity - $merge[1] >= 0) {
                ...
                alert()
                    ->success("تم التحويل من المستودع بنجاح !")
                    ->autoclose(5000);
            } else {
                alert()
                    ->warning("الكميه بالمستودع المنقول منه غير كافية")
                    ->autoclose(5000);
            } //endcheckif
        } //endforeach
    ...
    public function requests()
    {
        ...
        $requests = AccountingSroreRequest::where(
            "store_to",
            $current_store
        )->get();
        return view(
            "AccountingSystem.stores.store_requests",
            compact("requests")
        );
    }
    public function requests_all()
    {
        ...
        return view(
            "AccountingSystem.stores.store_requests_all",
            compact("requests")
        );
    }
    ...
    public function request($id)
    {
        ...
        return view(
            "AccountingSystem.stores.store_request",
            compact("transactions", "request")
        );
    }
    ...
    public function request_detail($id)
    {
        ...
        $request = AccountingSroreRequest::find($id);
        return view(
            "AccountingSystem.stores.store_request_detail",
            compact("transactions", "request")
        );
    }
    ...
    public function accept_request($id)
    {
        ...
        $tansactions = AccountingTransaction::where("request_id", $req->id)
        ...
        foreach ($tansactions as $product_id => $quantity) {
            ...
            alert()
                ->success("تم  قبول الاستلام التحويل من المستودع بنجاح !")
                ->autoclose(5000);
    ...
    public function refused_request(Request $request, $id)
    {
        ...
        $tansactions = AccountingTransaction::where("request_id", $req->id)
            ...
            ->toArray();
        foreach ($tansactions as $product_id => $quantity) {
            $product_store_from = AccountingProductStore::where(
                "store_id",
                $req->store_from
            )
                ->where("product_id", $product_id)
            ...
            alert()
                ->success("تم  رفض الاستلام التحويل من المستودع بنجاح !")
                ->autoclose(5000);
            return redirect()->route("accounting.stores.requests");
        }
    }
    ...
    public function damaged_store(Request $request)
    {
        ...
        $massage = [
            "quantity.required" => "اختر المستودع  ثم حددالاصناف التالفة",
        ];
        $this->validate($request, $rules, $massage);
        ...
        alert()
            ->success("تم   تسجيل التالف من المستودع بنجاح !")
            ->autoclose(5000);
    ...
    public function damaged_index()
    {
        ...
        return view(
            "AccountingSystem.stores.damaged_index",
            compact("damages")
        );
    }
    ...
    public function damaged_show($id)
    {
        ...
        $damage_products = AccountingDamageProduct::where(
            "damage_id",
            $id
        )->get();

        return view(
            "AccountingSystem.stores.show_damaged_products",
            compact("damage_products", "damage")
        );
    }
}
