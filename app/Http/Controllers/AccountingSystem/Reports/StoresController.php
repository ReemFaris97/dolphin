...
use App\DataTables\GeneralMovementsDataTable;
...
class StoresController extends Controller
{
    public function damages()
    {
        ...
        if (
            ...
            $stores_branch = AccountingStore::whereIn("model_id", $branches)
                ->where(
                    "model_type",
            ...
            //            dd($stores);
            $damages_id = AccountingDamage::whereIn("store_id", $stores)
                ->whereBetween("created_at", [$from, $to])
                ->pluck("id");
            $damages = AccountingDamageProduct::whereIn(
                "damage_id",
                $damages_id
            )->get();
        }
        ...
        elseif (
            ...
            $stores = AccountingStore::where("model_id", 1)
                ->where(
                    "model_type",
                    "App\Models\AccountingSystem\AccountingBranch"
                )
        ...
        elseif (
            ...
            $damages_id = AccountingDamage::where(
                "store_id",
                \request("store_id")
            )
                ->whereBetween("created_at", [$from, $to])
        ...
        elseif (
            ...
            $damages = AccountingDamageProduct::where(
                "product_id",
                \request("product_id")
            )
                ->whereBetween("created_at", [$from, $to])
        ...
        return view(
            "AccountingSystem.reports.stores.damagedProducts",
            compact("damages", "requests")
        );
    }
    ...
    public function inventory()
    {
        ...
        if (
            ...
            $inventory_id = AccountingInventory::whereIn("store_id", $stores)
                ->whereBetween("created_at", [$from, $to])
                ->pluck("id");
            $inventories = AccountingInventoryProduct::whereIn(
                "inventory_id",
                $inventory_id
            )->get();
            //             dd($inventories);
        ...
        elseif (
            ...
            $stores = AccountingStore::where("model_id", \request("branch_id"))
                ->where(
                    "model_type",
                    "App\Models\AccountingSystem\AccountingBranch"
                )
        ...
        elseif (
            ...
            $inventory_id = AccountingInventory::where(
                "store_id",
                \request("store_id")
            )
                ->whereBetween("created_at", [$from, $to])
        ...
        elseif (
            ...
            $inventories = AccountingInventoryProduct::where(
                "product_id",
                \request("product_id")
            )
                ->whereBetween("created_at", [$from, $to])
        ...
        return view(
            "AccountingSystem.reports.stores.InventoryProducts",
            compact("inventories", "requests")
        );
    }
    ...
    public function transactions()
    {
        ...
        if (
            ...
            $request_id = AccountingSroreRequest::whereIn("store_form", $stores)
                ->orWhereIn("store_to", $stores)
                ->whereBetween("created_at", [$from, $to])
                ->pluck("id");
            $transactions = AccountingTransaction::whereIn(
                "request_id",
                $request_id
            )->paginate(10);
        }
        ...
        elseif (
            ...
            $request_id = AccountingSroreRequest::where(
                "store_form",
                \request("store_id")
            )
                ->orwhere("store_to", \request("store_id"))
                ->orWhere("store_to", \request("store_id"))
                ->whereBetween("created_at", [$from, $to])
                ->pluck("id");
            $transactions = AccountingTransaction::whereIn(
        ...
        elseif (
            ...
            $transactions = AccountingTransaction::where(
                "product_id",
                \request("product_id")
            )
                ->whereBetween("created_at", [$from, $to])
        ...
        return view(
            "AccountingSystem.reports.stores.transactionProducts",
            compact("transactions", "requests")
        );
    }
    ...
    public function deficiency()
    {
        ...
        if (
            ...
            $product_quantity = AccountingProductStore::whereIn(
                "store_id",
                $stores
            ...
            foreach ($product_quantity as $key => $item) {
                ...
                $quantites[$product->id] = AccountingProductStore::where(
                    "product_id",
                    "=",
                    $product->id
                )
        ...
        return view(
            "AccountingSystem.reports.stores.deficiencyProducts",
            compact("deficiencies", "stores", "quantites", "requests")
        );
    }
    ...
    public function expirations()
    {
        ...
        return view(
            "AccountingSystem.reports.stores.expiredProducts",
            compact("expire_products", "quantites", "requests")
        );
    }
    ...
    public function stagnants()
    {
        ...
        if (
            ...
            foreach ($product_quantity as $key => $item) {
                ...
                $quantites[$product->id] = AccountingProductStore::where(
                    ...
                    ->whereIn("store_id", $stores)
                    ->sum("quantity");
                $last_item = AccountingSaleItem::where(
                    "product_id",
                    "=",
                    $product->id
                )
        ...
        return view(
            "AccountingSystem.reports.stores.stagnantProducts",
            compact("stagnant_sales", "quantites", "requests")
        );
    }
    ...
    public function movements(Request $request)
    {
        ...
        return view(
            "AccountingSystem.reports.stores.movementProducts",
            compact("purchases", "sales", "damages")
        );
    }

    public function generalMovements(GeneralMovementsDataTable $dataTable)
    {
        //        $accounting_products = AccountingProduct::query()
        //            ->haveMovementBetween($request->from ?? now()->toDateString(),$request->to ?? now()->toDateString())
        //            ->get();
        // return view('AccountingSystem.reports.stores.generalMovement', compact('accounting_products'));
        return $dataTable->render(
            "AccountingSystem.reports.stores.generalMovement"
        );
    }
}
