...
class SalesController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->input('company_id')) {
            ...
            $sales->selectRaw(
                "DATE(STR_TO_DATE(date,'%m/%d/%Y, %h:%i:%s %p')) as date_formatted , SUM(total) as total_amount , SUM(totalTaxs) as total_tax, Sum(total - totalTaxs) as total_without_taxes,count(id) sales_count"
            );
            //            dd($sales->get());
            ...
            if ($request->input('product_id') && $request->product_id != null) {
                $sales = $sales->whereHas('items', function ($item) use (
                    $request
                ) {
        ...
        return view(
            'AccountingSystem.reports.sales.for-period',
            compact('sales', 'requests')
        );
    }
    ...
    public function details()
    {
        ...
        //      dd($sales);
        return view(
            'AccountingSystem.reports.sales.sale-details',
            compact('sales')
        );
    }
    ...
    public function returnsPeriod(Request $request)
    {
        ...
        return view('AccountingSystem.reports.sales.returns-period', [
            'sales' => $sales,
            'requests' => $request->all(),
        ]);
    }
    ...
    public function returnsDay(Request $request)
    {
        ...
        return view(
            'AccountingSystem.reports.sales.returns-day',
            compact('sales', 'requests')
        );
    }
    ...
    {
        $sales = AccountingReturn::whereDate(
            'created_at',
            request('date')
        )->get();
        return view(
            'AccountingSystem.reports.sales.return-details',
            compact('sales')
        );
    }
    ...
    public function daily_earnings(Request $request)
    {
        $requests = request()->all();
        if ($request->input('company_id')) {
            $sales = AccountingSale::join(
                'accounting_sales_items',
                'accounting_sales.id',
                'accounting_sales_items.sale_id'
            )->select(
                'accounting_sales.id',
        ...
        return view(
            'AccountingSystem.reports.sales.daily-earnings',
            compact('sales', 'requests')
        );
    }
    ...
    public function period_earnings(Request $request)
    {
        ...
        $sales = AccountingSale::query()
            ...
            ->leftJoin(
                'accounting_products',
                'accounting_sales_items.product_id',
                'accounting_products.id'
            )
            ->select(
                'accounting_sales.id',
                DB::raw('DATE(accounting_sales.created_at) as date'),
                ...
                DB::raw('sum(accounting_sales.total) as all_total'),
                DB::raw(
                    'sum(accounting_sales_items.price *accounting_sales_items.quantity) as all_amounts'
                ),
                DB::raw(
                    'sum(accounting_sales_items.price_after_tax *accounting_sales_items.quantity) as all_amounts_after_tax'
                ),
                DB::raw('sum(accounting_sales.totalTaxs) as total_tax'),
                DB::raw('sum(accounting_sales.discount) as discounts'),
                DB::raw(
                    'sum(accounting_products.purchasing_price * accounting_sales_items.quantity ) as productPrice'
                ),
                'accounting_sales.created_at'
            );
        if ($request->input('branch_id') && $request->branch_id != null) {
        ...
        if ($request->input('from') && $request->input('to')) {
            $sales = $sales->whereBetween('accounting_sales.created_at', [
                Carbon::parse($request->from),
                Carbon::parse($request->to),
            ]);
        }
    ...
    public function sessionDetails(Request $request)
    {
        ...
        if (
            ...
            is_null($request->from_date) != null &&
            is_null($request->to_date)
        ) {
            $sales = $returns = collect([]);
            return view(
                'AccountingSystem.reports.sales.session-details',
                compact('sellers', 'sales', 'returns')
            );
        }
        $sales = AccountingSale::query()
            ->withCount('items')
    ...
    public function improvedSales(Request $request)
    {
        ...
        $sales->when($request->from and $request->to, function ($q) use (
            $request
        ) {
        ...
        return view(
            'AccountingSystem.reports.sales.sales-improved',
            compact('sales')
        );
    }
    ...
    public function improvedReturnSales(Request $request)
    {
        ...
        return view(
            'AccountingSystem.reports.sales.return_sales-improved',
            compact('sales')
        );
    }
}
