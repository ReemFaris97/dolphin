...
class TaskUserController extends Controller
{
    ...
    {
        $task_users = TaskUser::where("worker_finished_at", "!=", null)
            ->whereDate("finished_at", date("Y-m-d"))
            ->where("finisher_id", \Auth::id())
            ->get()
            ->reverse();
    ...
    {
        $task_users = TaskUser::notFinished()
            ->get()
            ->filter("oldFilter");
    ...
    {
        $task_users = TaskUser::/* where('finished_at', '!=', null)-> */ where(
            "rate",
            null
        )
