...
class ReportsController extends Controller
{
    public function getWorkerForm()
    {
        if (
            !auth()
                ->user()
                ->hasPermissionTo("view_workers_reports")
        ) {
            return abort(401);
        }
    ...
    public function workerReport(Request $request)
    {
        ...
        $rows = TaskUser::where("finished_at", "!=", null)
        ...
        if ($request->from != null) {
            $rows = $rows->filter(function ($task_user) use ($request) {
                ...
                return optional($task_user->finished_at)->greaterThanOrEqualTo(
                    $from_date
                );
            });
        }
        if ($request->to != null) {
            $rows = $rows->filter(function ($task_user) use ($request) {
                ...
                return optional($task_user->finished_at)->lessThanOrEqualTo(
                    $to_date
                );
            });
        }
    ...
    {
        $task_users = TaskUser::whereHas("task", function ($task) use (
            $request
        ) {
        ...
        return view(
            "admin.task_users.index",
            compact("task_users", "page_title")
        );
    }
}
