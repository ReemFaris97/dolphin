...
class SaleObserver
{
    public function creating(AccountingSale $sale)
    {
        if (getsetting("counter_type") == "daily") {
            $exiss = AccountingSale::whereDate("created_at", date("Y-m-d"))
                ->latest()
                ->first();
            if (!is_null($exiss)) {
            ...
            if (!is_null($exiss)) {
                ...
                $sale->counter_sale =
                    $sale->daily_number . "-" . $sale->branch->code;
            } else {
                $sale->daily_number = 1;
                $sale->counter_sale =
                    $sale->daily_number . "-" . $sale->branch->code;
            ...
            if (!is_null($exiss)) {
                ...
                $sale->daily_number = 1;
                $sale->counter_sale =
                    $sale->daily_number . "-" . optional($sale->branch)->code ??
                    "" . "-" . $sale->session->device->code;
            }
        }
    }
