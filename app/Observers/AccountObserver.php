    ...
    public function creating(AccountingAccount $account)
    {
        ...
        if (getsetting("automatic") == "1") {
            if ($account->kind == "main") {
                $lastMainAcount = AccountingAccount::where("kind", "main")
                    ->latest()
                    ->first();
                ...
                $lastfollowingAcount = AccountingAccount::where(
                    "kind",
                    "following_main"
                )
                ...
                if (!is_null($lastfollowingAcount)) {
                    $account->code =
                        optional($lastfollowingAcount)->code + 1000;
                ...
                $perantAccount = AccountingAccount::find($account->account_id);
                $lastsubAcount = AccountingAccount::where("kind", "sub")
                    ->where("account_id", $account->account_id)
                    ->latest()
                    ->first();
        ...
        ////////////////////////غير تلقائى-------------تسلسلى
        elseif (
            getsetting("automatic") == "0" &&
            getsetting("coding_status") == "0"
        ) {
            if ($account->kind == "sub" || $account->kind == "following_main") {
                $perantAccount = AccountingAccount::find($account->account_id);
                $lastsubAcount = AccountingAccount::whereIn("kind", [
                    "following_main",
                    "sub",
                ])
                    ->where("account_id", $account->account_id)
                ...
                if (!is_null($lastsubAcount)) {
                    $account->code =
                        optional($lastsubAcount)->code +
                        getsetting("increased_number");
                    $account->level = optional($lastsubAcount)->level;
                } else {
                    $account->code =
                        optional($perantAccount)->code .
                        getsetting("increased_number");
                    $account->level = optional($perantAccount)->level + 1;
                }
            }
