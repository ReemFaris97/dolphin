...
class TripInventory extends Model
{
    ...
    public function trip_report(): HasOne
    {
        return $this->hasOne(
            ...
            "route_trip_id",
            "trip_id"
        )->whereColumn("route_trip_reports.round", "round");
    }
    ...
    public function scopeFilterDistributors(
        Builder $builder,
        $distributors = null
    ): void {
        $builder->when($distributors != null, function ($q) use (
            $distributors
        ) {
    ...
    public function getProductItemsAttribute()
    {
        ...
        foreach ($this->products ?? [] as $product) {
            ...
            if ($this->previous_trip_report != null) {
                $pervious_sells = $this->previous_trip_report->products
                    ->where("product_id", $product->product_id)
                    ->sum("quantity");
                $pervious_exists = $this->previous_trip_inventory->products
                    ->where("product_id", $product->product_id)
                    ->sum("quantity");
            }
