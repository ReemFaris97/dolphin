...
class TaskUser extends Model
{
    ...
    {
        return $this->belongsTo("App\Models\User", "rater_id")->withDefault([
            "name" => "لا يوجد مقيم",
        ]);
    }
    ...
    {
        return $this->belongsTo("App\Models\User", "finisher_id")->withDefault([
            "name" => "لا يوجد مسئول عن الانتهاء",
        ]);
    }
    ...
    public function getTotalDurationAttribute()
    {
        if (!isset($this->attributes["total_duration"])) {
            return TaskUser::where("task_id", $this->task_id)
                ->where("id", "<=", $this->id)
                ->orderBy("id", "asc")
                ->sum("task_duration");
        }
    ...
    {
        return optional(optional($this->task)->date_with_time)->addSeconds(
            $this->total_duration
        );
    }
    ...
    public function getFromTimeAttribute()
    {
        ...
        return optional($this->task)
            ->date_with_time->addSeconds($this->total_duration)
            ->subSeconds($this->task_duration);
    }
    ...
    public function getStatusAttribute()
    {
        ...
        if (
            Carbon::now()->greaterThan($this->from_time) &&
            Carbon::now()->greaterThan($this->to_time)
        ) {
            return "old";
        }
    ...
    {
        $finished_users = TaskUser::where("task_id", $this->task_id)
            ->where("finished_at", "!=", null)
            ->get();
    ...
    public function getFinishingRateAttribute()
    {
        ...
        $diff_in_Seconds = optional($this->from_time)->diffInSeconds(
            $this->finished_at
        );
        if ($diff_in_Seconds <= $duration_quarter) {
            return "ممتاز";
        } elseif (
            $duration_quarter < $diff_in_Seconds &&
            $diff_in_Seconds <= $duration_quarter * 2
        ) {
            return "جيد جدا";
        } elseif (
            $duration_quarter * 2 < $diff_in_Seconds &&
            $diff_in_Seconds <= $duration_quarter * 3
        ) {
            return "جيد ";
        } elseif (
            $duration_quarter * 3 < $diff_in_Seconds &&
            $diff_in_Seconds <= $duration_quarter * 4
        ) {
            return "مقبول ";
        } elseif (
            $duration_quarter * 4 < $diff_in_Seconds &&
            $diff_in_Seconds <= $duration_quarter * 5
        ) {
            return "سئ";
        }
    }
