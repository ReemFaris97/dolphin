...
class Task extends Model
{
    ...
    {
        return $this->hasMany("App\Models\TaskUser", "task_id")->with(
            "user",
            "rater",
            "finisher"
        );
    ...
    {
        return $this->hasMany("App\Models\TaskLog", "task_id")->with(
            "old_user",
            "new_user"
        );
    }
    ...
    public function currentTask()
    {
        ...
        foreach ($this->user_tasks as $task_user) {
            if ($task_user->full_date >= new \DateTime(now())) {
                break;
            } else {
                continue;
            }
    ...
    public function scopePresent($query, $user_id = null, $assigned_only = null)
    {
        $user_tasks = TaskUser::where("finished_at", null)
            ...
            ->has("task")
            ->when($user_id != null, function ($task_user) use (
                $user_id,
                $assigned_only
            ) {
    ...
    {
        $user_tasks = TaskUser::when($user_id != null, function (
            $task_user
        ) use ($user_id, $assigned_only) {
            $task_user->OfUser($user_id, $assigned_only);
        })
            ->has("task")
            ->get();
    ...
    {
        $user_tasks = TaskUser::whereNotNull("worker_finished_at")
    ...
    public function getDateWithTimeAttribute()
    {
        $date = is_null($this->date)
            ? $this->created_at->addDays(30)->format("Y-m-d")
            : $this->date->toDateString();
        $time_from = is_null($this->time_from)
            ? "00:00:00"
            : date("H:i:s", strtotime($this->time_from));
    ...
    {
        $tasks_pre = Task::present(auth()->user()->id)
            ->pluck("id")
            ->toArray();
        $tasks_fut = Task::future(auth()->user()->id)
            ->pluck("id")
            ->toArray();
        $tasks_old = Task::old(auth()->user()->id)
            ->pluck("id")
            ->toArray();
        $tasks_to_fin = Task::toFinish(auth()->user()->id)
            ->pluck("id")
            ->toArray();
        $tasks_to_rat = Task::toRate(auth()->user()->id)
            ->pluck("id")
            ->toArray();
        // dd($tasks_old);
