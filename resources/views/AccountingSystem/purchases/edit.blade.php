...
@section('title','تعديل فاتوره مشتري')
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12">
                            <div class="form-group  {{(getsetting('show_supplier_balance')==1) ? 'show_supplier_balance_enable col-sm-2':'col-sm-4' }}">
                                ...
                                {!! Form::select("supplier_id",$suppliers,$purchase->supplier_id,['class'=>'selectpicker form-control inline-control','placeholder'=>'اختر اسم المورد','data-live-search'=>'true','id'=>'supplier_id'])!!}
                            </div>
...
{{--                            @endif--}}
                            <div class="form-group col-sm-3">
                                ...
                                {!! Form::text("bill_num",$purchase->bill_num,['class'=>'selectpicker form-control inline-control','placeholder'=>' رقم الفاتوره',"id"=>'bill_num'])!!}
                            </div>
                            <div class="form-group col-sm-3">
                                <label for="bill_date"> تاريخ الفاتورة </label>
                                {!! Form::text("__bill_date",$purchase->bill_date,['class'=>'inlinedatepicker form-control inline-control','placeholder'=>' تاريخ الفاتورة',"id"=>'bill_date'])!!}
                            </div>
                        </div>
                    ...
                    {{-- <form method="update" id="buyForm" action="{{route('accounting.purchases.update',$purchase->id)}}">
                       ...
                       {{(getsetting('name_enable')==1) ? 'name_enable':'' }}
                        ...
                        {{(getsetting('total_price_after_enable')==1) ? 'total_price_after_enable':'' }}
                            ...
                            <tr class="single-row-wrapper"  data-ifhastax="${priceHasTax}" data-tot-taxes="${totalTaxes}">
                                <td class="row-num">{{++$key}}</td>
                                <input type="hidden" name="product_id[]" value="{{$row->product_id}}">
                                <td class="product-name maybe-hidden name_enable" width="230"><a href="{{route('accounting.products.show',['id'=>$row->product_id])}}" target="_blank" rel="noopener noreferrer">{{$row->product->name}}</a></td>
                                <td class="product-unit maybe-hidden unit_enable">
{{--                                          @dd($row->units())--}}
                                    <select class="form-control js-example-basic-single" name="unit_id[{{$row->product_id}}]" >
                                        @foreach($row->units() as $unit)
                                          <option data-uni-price="{{$unit->purchasing_price}}" value="{{$unit->product_id??$unit->id}}"
                                ...
                                <td class="product-quantity maybe-hidden quantity_enable">
                                    <input type="number" placeholder="الكمية" step="1" min="1" value="{{$row->quantity}}" id="sale" class="form-control" name="quantity[{{$row->product_id}}]">
                                </td>
                                <td class="expiration-date maybe-hidden expiration_enable">
                                    <input type="date" class="expiration form-control" name="expire_date"  value="{{$row->expire_date}}">
                                </td>
                                <td class="single-price-before maybe-hidden unit_price_before_enable">
                                    <input type="number" class="form-control" step="any" value="{{$row->price}}" name="prices[{{$row->product_id}}]">
                                </td>
                                <input type="hidden" name="itemTax[{{$row->product_id}}]" value="{{$row->tax}}">
                                <td class="single-price-after maybe-hidden unit_price_after_enable" data-sinAft="{{$row->price_after_tax}}">
                                    {{$row->tax}}
                                </td>
                                <td class="whole-price-before maybe-hidden total_price_before_enable">{{$row->price}}</td>
                                <td class="whole-price-after maybe-hidden total_price_after_enable">{{$row->price_after_tax}}</td>
