        ...
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                ...
                <section class="yourBill">
                    ...
                    {{--            @dd(auth()->user()->accounting_store_id)--}}
                        ...
                        <div class="row table-upper-options">
                            ...
                            <div class="form-group block-gp col-md-4 col-sm-4 col-xs-12">
                                ...
                                {!! Form::text("__bill_date",$sale->bill_date,['class'=>'inlinedatepicker form-control inline-control','placeholder'=>' تاريخ الفاتورة',"id"=>'bill_date'])!!}
                            </div>
                    ...
                    <div class="result">
                        {!!Form::model($sale, ['route' => ['accounting.sales.update' ,$sale->id] ,'class'=>'phone_validate','id'=>'sllForm','method' => 'PATCH','files'=>true]) !!}
                        @csrf
                            ...
                            >
                                    ...
                                    <tr class="single-row-wrapper" id="row{{ ++$key }}">
                                        ...
                                        <td class="product-unit maybe-hidden unit_enable" width="110">
                                            <select class="form-control js-example-basic-single" name="unit_id_old[{{$item->product_id}}]">
                                                ...
                                                @if($item->unit_type=='sub' && $item->unit_id=$unit->id)
                                                   selected
                                                    @endif
                                                >{{$unit->name}}</option>
                                              @endforeach
                                        ...
                                        <td class="product-quantity maybe-hidden quantity_enable" width="100">
                                            <input type="number" placeholder="الكمية" max="" min="1" value="{{$item->quantity}}" id="sale" name="quantity_old[{{$item->product_id}}]" class="form-control">
                                        </td>
                                        <td class="unit-total-tax maybe-hidden unit_total_tax_enable" width="100">
                                            <input type="number" placeholder="الضريبة" max="" min="0" data-original-tax="${totalTaxes}" value="{{$item->tax}}" name="tax[]" class="form-control">
                                        </td>
                                ...
                                <tr>
                                    <th id="amountBeforeDariba" class="rel-cols" colspan="3">
                                        ...
                                        <span class="dynamic-span">{{$sale->amount}}</span>
									...
									<span id="removeTaxWrap">
                                        ...
                                        <span class="dynamic-span">{{round($sale->totalTaxs,2)}}</span>
                                        <span class="rs"> ر.س </span>
                                    </th>
                                    <th id="amountAfterDariba" class="rel-cols" colspan="3">
                                        ...
                                        <span class="dynamic-span">{{$sale->total}}</span>
                                        <span class="rs"> ر.س </span>
                                    </th>
                                </tr>
                                <tr id="discountArea">
...
@section('scripts')
    ...
    <script>
        ...
        function calcBill(selectedProduct, productId, productName, productBarCode, productPrice, priceHasTax, totalTaxes, mainUnit, productUnits ) {
            ...
            <input  type="hidden" name="price_after_tax[]" value= ${singlePriceAfter.toFixed(rondingNumber)}>
