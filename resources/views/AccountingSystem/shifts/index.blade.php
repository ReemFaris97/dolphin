            ...
            <table class="table datatable-button-init-basic">
                ...
                @foreach($shifts as $row)
                    <tr>
                        ...
                        <td>{!! $row->name!!}</td>
                        <td><a href="{{route('accounting.branches.show',$row->branch_id)}}">{!! optional($row->branch)->name!!}</a></td>
                        <td>{!! $row->from!!}</td>
                        <td>{!! $row->to!!}</td>


                        <td class="text-center">
                            <a href="{{route('accounting.shifts.edit',$row->id)}}" data-toggle="tooltip" data-original-title="تعديل"> <i class="icon-pencil7 text-inverse" style="margin-left: 10px"></i> </a>
