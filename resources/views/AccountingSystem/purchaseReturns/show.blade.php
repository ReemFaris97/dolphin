...
@section('title','عرض فاتوره مرتجع'.' '. $purchaseReturn->id)
			...
			<div class="card-body ">
				...
				<div class="container-fluid the-bill-design too-be-topped" id="div-to-print">
					...
					<div class="one-bill-inpt the-bill-numbere the-bill-number">
						...
						{!! $purchaseReturn->counter_purchase !!}
					</div>
					<div class="flex-col">
						...
						<div class="flex-col mar-top-15">
							<table class="tablesaw bill-table-whole-wrapper table-bordered table-hover table" data-tablesaw-mode="stack" data-tablesaw-sortable data-tablesaw-sortable-switch data-tablesaw-minimap data-tablesaw-mode-switch>
								...
								<tr class="bill-table-tr-wrapper fixed-ta-hd">
									<th rowspan="1" class="fixed-ta-hd maybe-hidden total_price_before_enable">قبل الضريبة</th>
						...
						<tr class="single-row-wrapper">
							...
							<td class="whole-price-after maybe-hidden total_price_after_enable">{{$row->price_after_tax*$row->quantity}}</td>
									...
									<tr>
										...
										<td colspan="7"><span class="tot-money">{!! $purchaseReturn->amount+$purchaseReturn->totalTaxs??0 !!}</span></td>
									</tr>
									<tr>
										<td data-tablesaw-sortable-col data-tablesaw-priority="6" colspan="4"><i class="ti-minus"></i> نوع الخصم</td>
										<td colspan="7">
											@if($purchaseReturn->discount_type=='percentage')
											نسبه
											@else
											مبلغ
											@endif
									...
									<tr>
										...
										<td colspan="7">
											@if($purchaseReturn->discount_type=='percentage')
											{!! ($purchaseReturn->discount* $purchaseReturn->amount)/100 ??0 !!}
												@else
									...
									<tr>
										...
										<td colspan="7">{!! $purchaseReturn->total??0 !!}</td>
										@else
										<td colspan="7">{!! $purchaseReturn->payed ??0 !!}</td>
										@endif
									</tr>
									<tr>
										...
										<td colspan="7">0</td>
										@else
										<td colspan="7">{!! ($purchaseReturn->total- $purchaseReturn->payed)??0 !!}</td>
										@endif
									</tr>
								</tbody>
							</table>
