...
@section('styles')
    <!--- start datatable -->
    <link href="https://cdn.jsdelivr.net/gh/guillaumepotier/Parsley.js@2.9.2/doc/assets/docs.css" rel="stylesheet"
        type="text/css">
    ...
    <div class="panel panel-flat" id="container">
        <div class="panel-heading">
            ...
            <h5 class="panel-title">
                ...
                <a href="{{ url('/accounting/settings/purchases_bill') }}" class="btn btn-success bill-cogs"
                    ...
                    <div class="yurSections">
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                ...
                                <div
                                    class="form-group  {{ getsetting('show_supplier_balance') == 1 ? 'show_supplier_balance_enable col-sm-2' : 'col-sm-4' }}">
                            ...
                            <div class="col-md-4 col-sm-4 col-xs-12 pos-rel">
                                ...
                                <a href="{{ route('accounting.products.create') }}" target="_blank"
                                    class="btn btn-primary pos-abs-btn">
                                    اضافه منتج جديد
                                </a>
                            </div>
                                ...
                                @if (Request::is('*/edit'))
                                    @foreach ($product_items as $key => $row)
                                        <tr class="single-row-wrapper" id="row{{ ++$key }}"
                                            data-ifhastax="{{ $row->tax == 0 ? 0 : 1 }}"
                                            data-tot-taxes="{{ $row->tax }}">
                                            ...
                                            <td class="product-unit maybe-hidden unit_enable" width="70">
                                                <select class="form-control js-example-basic-single"
                                                    name="unit_id_old[{{ $row->product_id }}]">
                                                    @foreach ($row->units() as $unit)
                                                        <option value="{{ $unit->product_id ?? $unit->id }}" @if ($row->unit_type == 'sub' && ($row->unit_id = $unit->id))
                                                            selected
                                                    @endif
                                                    >{{ $unit->name }}</option>
                                    @endforeach
                                    ...
                                    <td class="product-quantity maybe-hidden quantity_enable" width="70">
                                        <input type="number" placeholder="الكمية" step="1" min="1" value="1" id="sale"
                                            class="form-control" name="quantity_old[{{ $row->product_id }}]">
                                    </td>
                                    ...
                                    <td class="unit-price maybe-hidden unit_price_enable" width="70">
                                        <input type="number" class="form-control" step="any"
                                            value="{{ round($row->price, 3) }}" name="">
                                    </td>
                                    <td class="unit-total-tax maybe-hidden unit_total_tax_enable" width="100">
                                        <input type="number" placeholder="الضريبة"
                                            data-original-tax="{{ $row->product->total_taxes }}"
                                            value="{{ $row->product->total_taxes }}" name="tax[]" class="form-control">
                                    </td>
                                    <td class="quantityXprice maybe-hidden total_enable" width="70">{{ $row->price }}
                                    </td>
                                    <td class="whole-product-gifts maybe-hidden gifts_enable" width="70">
                                        <input type="number" placeholder="الهدايا" step="1" min="0" value="0"
                                            class="form-control" name="gifts_old[{{ $row->gifts }}]">
                                    </td>
                                    ...
                                    <td class="single-price-before maybe-hidden">
                                        <input type="number" class="form-control" step="any" value="{{ $row->price }}"
                                            name="prices_old[{{ $row->price }}]">
                                    </td>

                                    <input type="hidden" name="itemTax[{{ $row->product_id }}]"
                                        value="{{ $row->tax }}">
                                    <td class="single-price-after maybe-hidden total_taxes_enable"
                                        data-sinAft="{{ $row->price_after_tax }}" width="70">
                                        {{ $row->tax }}
                                    </td>
                                    <td class="whole-price-before maybe-hidden ">{{ $row->price }}</td>
                                    <td class="whole-price-after maybe-hidden total_pure_enable" width="70">
                                        {{ $row->price_after_tax }}</td>

                                    <td class="bill-operations-td maybe-hidden operations_enable" width="160">

                                        <a href="#" title="مسح" class="remove-prod-from-list"><span
                                    ...
                                    <th colspan="3" class="rel-cols" id="discountArea">
                                        ...
                                        <div class="inline_divs">
                                            <div class="form-group">
                                                <div>
                                                    <label for="byPercentage" class="wit-lbl">ادخل نسبة
                                                        الخصم</label>
                                                    <input type="number" step="any" placeholder="النسبة المئوية للخصم"
                                                        min="0" value="0" max="100" id="byPercentage"
                                                        name="discount_byPercentage" class="form-control dynamic-input">
                                                    <span class="rs"> % </span>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                    ...
                                    <th id="paymentMethod" colspan="3" class="rel-cols">
                                        ...
                                        <div class="inline_divs">
                                            <div class="form-group rel-cols radiBtnwrap">
                                                <input type="radio" checked id="tazaBTaza" name="payment" value="cash"
                                                    required="" data-parsley-required-message="من فضلك اختر طريقة الدفع">
...
if (!Number.prototype.$truncate) {
  Object.defineProperty(Number.prototype, '$truncate', {
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    calculateBill(resp.id, resp.name, resp.link, parseFloat(resp.last_price),
                        parseFloat(resp.average), resp.bar_code, parseFloat(resp.price), resp
                        .price_has_tax, resp.total_taxes, JSON.parse(resp.subunits), resp
                        .product_expiration)
        ...
        function calculateBill(ProductId, productName, productLink, lastPrice, avgPrice, barCode, productPrice, priceHasTax,
            totalTaxes, productUnits, expirationDate) {
            var dateInpt = '';
            ...
            if (parseFloat(priceHasTax) === 0) {
                ...
                var singlePriceAfter = parseFloat(productPrice) + (parseFloat(productPrice) * (parseFloat(totalTaxes) /
                    100));
            } else if (parseFloat(priceHasTax) === 1) {
                var onllyDariba = parseFloat(productPrice) - (parseFloat(productPrice) * (100 / (100 + parseFloat(
                    totalTaxes))));
            ...
            var netTax = parseFloat(parseFloat(singlePriceAfter) - parseFloat(singlePriceBefore)).$truncate(rondingNumber);
            ...
            $("#discMod" + rowNum + "  a.appendAnewDiscount").on('click', function() {
                ...
                $(".singleSpecialDiscByPer").each(function() {
                    $(this).on('change', function() {
                        $(this).parents('.single-special-dis-wrap').find('.singleSpecialDiscByVal')
                            .val(0);
                        if (($(this).val()) < 0) {
                            $(this).val(0);
                            $(this).text(0);
                ...
                $(".singleSpecialDiscByVal").each(function() {
                    $(this).on('change', function() {
                        $(this).parents('.single-special-dis-wrap').find('.singleSpecialDiscByPer')
                            .val(0);
                        if (($(this).val()) < 0) {
                            $(this).val(0);
                            $(this).text(0);
            ...
            $(".product-unit select").change(function() {
                ...
                if (parseFloat(priceHasTax) === 0) {
                    var singlePriceBefore = parseFloat(productPrice);
                    var singlePriceAfter = parseFloat(productPrice) + (parseFloat(productPrice) * (parseFloat(
                        totalTaxes) / 100));
                } else if (parseFloat(priceHasTax) === 1) {
                    var onllyDariba = parseFloat(productPrice) - (parseFloat(productPrice) * (100 / (100 +
                ...
                var theUnitPrice = +($(this).parents("tr.single-row-wrapper").find(".single-price-before input")
                    .val());
                ...
                $(this).parents('.single-row-wrapper').find(".single-price-before input").val(singlePriceBefore
                    .$truncate(rondingNumber));
                $(this).parents('.single-row-wrapper').find(".single-price-after").attr('data-sinAft',
                    singlePriceAfter.$truncate(rondingNumber));
                $(this).parents('.single-row-wrapper').find(".single-price-after").text(netTax.$truncate(
                    rondingNumber));
                var wholePriceBefore = parseFloat($(this).parents('.single-row-wrapper').find(
                    ".single-price-before input").val()) * parseFloat($(this).parents('.single-row-wrapper')
                    .find(".product-quantity input").val());
                $(this).parents('.single-row-wrapper').find(".whole-price-before").text(wholePriceBefore.$truncate(
                    rondingNumber));
                $(this).parents('.single-row-wrapper').find(".whole-price-before").attr('tempPriBef',
                    wholePriceBefore.$truncate(rondingNumber));
                var wholePriceAfter = parseFloat(
                    ...
                    parseFloat(netTax).$truncate(rondingNumber));

                $(this).parents('.single-row-wrapper').find(".whole-price-after").text(parseFloat(parseFloat(
                    wholePriceAfter) + parseFloat(netTax)).$truncate(rondingNumber));
            ...
            $(".product-quantity input").change(function() {
                ...
                +($(this).parents('.single-row-wrapper').find(".single-price-after").text(netTax.$truncate(
                    rondingNumber)));
                var wholePriceBefore = parseFloat($(this).parents('.single-row-wrapper').find(
                    ".single-price-before input").val()) * parseFloat($(this).val());
                +($(this).parents('.single-row-wrapper').find(".whole-price-before").text(wholePriceBefore.$truncate(
                    rondingNumber)));
                var wholePriceAfter = parseFloat($(this).parents('.single-row-wrapper').find(".single-price-after")
                    .attr('data-sinAft')) * parseFloat($(this).val());

                +($(this).parents('.single-row-wrapper').find(".whole-price-after").text(
                    parseFloat(parseFloat(wholePriceAfter.$truncate(rondingNumber)) + parseFloat(netTax).$truncate(
                        rondingNumber))));
                +($(this).parents('.single-row-wrapper').find(".whole-price-before").attr('tempPriBef',
                    wholePriceBefore.$truncate(rondingNumber)));
                $('.special-discount-modal').trigger('hidden.bs.modal');
            ...
            $(".unit-price input").change(function() {
                ...
                if (parseFloat(priceHasTax) === 0) {
                    var singlePriceBefore = parseFloat(productPrice);
                    var singlePriceAfter = parseFloat(parseFloat(productPrice) + (parseFloat(productPrice) * (
                        parseFloat(totalTaxes) / 100)));
                } else if (parseFloat(priceHasTax) === 1) {
                    var onllyDariba = parseFloat(parseFloat(productPrice) - (parseFloat(productPrice) * (100 / (
                        100 + parseFloat(totalTaxes)))));
                ...
                var theQuantity = parseFloat($(this).parents("tr.single-row-wrapper").find(
                    ".product-quantity input").val());
                var theUnitPrice = parseFloat($(this).parents("tr.single-row-wrapper").find(
                    ".single-price-before input").val());
                var theUnitTax = parseFloat($(this).parents("tr.single-row-wrapper").find(".unit-total-tax input")
                    .val());
                var netTax = parseFloat((parseFloat(theUnitTax) / 100) * parseFloat(theQuantity) * parseFloat(
                    theUnitPrice));
                ...
                $(this).parents('.single-row-wrapper').find(".single-price-after").text(netTax.$truncate(
                    rondingNumber));
                var wholePriceBefore = parseFloat($(this).parents('.single-row-wrapper').find(
                    ".product-quantity input").val()) * parseFloat($(this).val());
                $(this).parents('.single-row-wrapper').find(".whole-price-before").text(wholePriceBefore.$truncate(
                    rondingNumber));
                var wholePriceAfter = parseFloat($(this).parents('.single-row-wrapper').find(".single-price-after")
                    .attr('data-sinAft')) * parseFloat($(this).parents('.single-row-wrapper').find(
                    ".product-quantity input").val());
                $(this).parents('.single-row-wrapper').find(".whole-price-after").text((parseFloat(
                    wholePriceAfter) + parseFloat(netTax)).$truncate(rondingNumber));
                $(this).parents('.single-row-wrapper').find(".whole-price-before").attr('tempPriBef',
                    wholePriceBefore.$truncate(rondingNumber));
                $(this).parents("tr.single-row-wrapper").find(".product-quantity input").trigger('change');
            ...
            function calcInfo() {
                ...
                $("#amountBeforeDariba span.dynamic-span").html(amountBeforeDariba.$truncate(rondingNumber));
                $("#amountAfterDariba span.dynamic-span").html(amountAfterDariba.$truncate(rondingNumber));
                $("#amountOfDariba span.dynamic-span").html(amountOfDariba.$truncate(rondingNumber));
                ...
                if (byAmount == 0 && byPercentage == 0) {
                    $("#demandedAmount span.dynamic-span").html(amountAfterDariba.$truncate(rondingNumber));
                    ...
                    $("input#byPercentage").change(function() {
                        ...
                        total = parseFloat(parseFloat(amountAfterDariba) - (parseFloat(amountAfterDariba) * (
                            parseFloat($(this).val()) / 100)));
                        $("#demandedAmount span.dynamic-span").html(total.$truncate(rondingNumber));
                        $("#demandedAmount1").val(total);
                    });
                    $("input#byAmount").change(function() {
                        if ((parseFloat($(this).val())) > parseFloat($("#amountAfterDariba span.dynamic-span")
                                .html())) {
            ...
            $('#discMod' + rowNum).on('hide.bs.modal', function(e) {
                ...
                var theSingleTax = parseFloat(parseFloat((parseFloat(theUnitTax) / 100) * parseFloat(theQuantity) *
                    parseFloat(theUnitPrice)));
                $('#row' + onlyModNum).find(".single-price-after").text(theSingleTax.$truncate(rondingNumber));
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    if (i === 0) {
                        if (($(rows[0]).find('.singleSpecialDiscByVal').val()) != 0) {
                            $("tr#row" + onlyModNum).find(".bud1").html(parseFloat($(rows[0]).find(
                                '.singleSpecialDiscByVal').val()).$truncate(rondingNumber) + 'ريال')
                            $("tr#row" + onlyModNum).find(".per1").html('---')
                        }
                    }
                    if (i === 1) {
                        if (($(rows[1]).find('.singleSpecialDiscByVal').val()) != 0) {
                            $("tr#row" + onlyModNum).find(".bud2").html(parseFloat($(rows[1]).find(
                                '.singleSpecialDiscByVal').val()).$truncate(rondingNumber) + 'ريال')
                            $("tr#row" + onlyModNum).find(".per2").html('---')
                        }
                    }
  ...
  let money=
                    ...
                    finalAftDisc -= parseFloat(parseFloat(parseFloat($(rows[i]).find('.singleSpecialDiscByPer')
                        .val()) / 100) * finalAftDisc);
                    $('#row' + onlyModNum).find('.whole-price-before').text(finalAftDisc.$truncate(rondingNumber));
                    if ($(rows[i]).find(".effectTax").is(":checked")) {
                        ...
                        $('#row' + onlyModNum).find('.single-price-after').text(newNetTax.$truncate(rondingNumber));
            ...
            /*        $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#selectID').change(function () {
                                ...
                                var lastPrice = parseFloat(selectedProduct.data('last-price')).$truncate(rondingNumber);
                                var avgPrice = parseFloat(selectedProduct.data('average')).$truncate(rondingNumber);
        ...
        function byBarcode() {
            ...
            var productUnits = selectedProduct.data('subunits');
            var expirationDate = selectedProduct.data('product_expiration');
            calculateBill(ProductId, productName, productLink, lastPrice, avgPrice, barCode, productPrice, priceHasTax,
                totalTaxes, productUnits, expirationDate)
        }
