...
<table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_1">
    ...
    @foreach($suppliers_discards as $row)
        <tr>
            ...
            <td>{{$row->date}}</td>
            <td>
                <a href="{!!route('supplier.suppliers-discards.show',$row->id)!!}" class="btn btn-info"> <i class="fas fa-eye"></i>مشاهده</a>
                <form method="POST" action="{!!route('supplier.suppliers-discards.destroy',$row->id)!!}">
                    @csrf() @method('delete')
