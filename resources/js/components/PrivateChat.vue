...
<!--                </v-flex>-->
                ...
                <v-flex xs1 class="text-center">
                    <file-upload
                            :post-action="'/admin/private-messages/'+activeFriend"
                            ref='upload'
                            v-model="files"
                            @input-file="$refs.upload.active = true"
                            :headers="{'X-CSRF-TOKEN': token}"
                    >
                        <i class="icon flaticon-attachment"></i>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      fetchMessages() {
         if(!this.activeFriend){
          return alert('Please select friend');
            ...
            axios.get('/admin/private-messages/'+this.activeFriend).then(response => {
      ...
      fetchUsers() {
            axios.get('/admin/chat-users').then(response => {
                this.users = response.data;

                if(this.friends.length>0){
                  this.activeFriend=this.friends[0].id;
                    this.activeUserName=this.friends[0].name;
                }
            });
        },
